    ...
    public function getTabs(): array
    {
        ...
        return [
            'all' => Tab::make('Semua Kuota')
                ...
                ->badge(WeeklyQuota::count()),

            'today' => Tab::make('Hari Ini')
                ->icon('heroicon-m-calendar')
                ->modifyQueryUsing(fn (Builder $query) => $query->where('day_of_week', $todayDayOfWeek))
                ->badge(WeeklyQuota::where('day_of_week', $todayDayOfWeek)->count())
                ->badgeColor('success'),

            'monday' => Tab::make('Senin')
                ->icon('heroicon-m-calendar-days')
                ->modifyQueryUsing(fn (Builder $query) => $query->where('day_of_week', 'monday'))
                ->badge(WeeklyQuota::where('day_of_week', 'monday')->count())
                ->badgeColor($todayDayOfWeek === 'monday' ? 'success' : 'gray'),

            'tuesday' => Tab::make('Selasa')
                ->icon('heroicon-m-calendar-days')
                ->modifyQueryUsing(fn (Builder $query) => $query->where('day_of_week', 'tuesday'))
                ->badge(WeeklyQuota::where('day_of_week', 'tuesday')->count())
                ->badgeColor($todayDayOfWeek === 'tuesday' ? 'success' : 'gray'),

            'wednesday' => Tab::make('Rabu')
                ->icon('heroicon-m-calendar-days')
                ->modifyQueryUsing(fn (Builder $query) => $query->where('day_of_week', 'wednesday'))
                ->badge(WeeklyQuota::where('day_of_week', 'wednesday')->count())
                ->badgeColor($todayDayOfWeek === 'wednesday' ? 'success' : 'gray'),

            'thursday' => Tab::make('Kamis')
                ->icon('heroicon-m-calendar-days')
                ->modifyQueryUsing(fn (Builder $query) => $query->where('day_of_week', 'thursday'))
                ->badge(WeeklyQuota::where('day_of_week', 'thursday')->count())
                ->badgeColor($todayDayOfWeek === 'thursday' ? 'success' : 'gray'),

            'friday' => Tab::make('Jumat')
                ->icon('heroicon-m-calendar-days')
                ->modifyQueryUsing(fn (Builder $query) => $query->where('day_of_week', 'friday'))
                ->badge(WeeklyQuota::where('day_of_week', 'friday')->count())
                ->badgeColor($todayDayOfWeek === 'friday' ? 'success' : 'gray'),

            'saturday' => Tab::make('Sabtu')
                ->icon('heroicon-m-calendar-days')
                ->modifyQueryUsing(fn (Builder $query) => $query->where('day_of_week', 'saturday'))
                ->badge(WeeklyQuota::where('day_of_week', 'saturday')->count())
                ->badgeColor($todayDayOfWeek === 'saturday' ? 'success' : 'gray'),

            'sunday' => Tab::make('Minggu')
                ->icon('heroicon-m-calendar-days')
                ->modifyQueryUsing(fn (Builder $query) => $query->where('day_of_week', 'sunday'))
                ->badge(WeeklyQuota::where('day_of_week', 'sunday')->count())
                ->badgeColor($todayDayOfWeek === 'sunday' ? 'success' : 'gray'),
        ];
    }
}